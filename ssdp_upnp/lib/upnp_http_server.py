...
class UPNPHTTPServerHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/airmentor_wsd.xml':
            self.send_response(200)
            self.send_header('Content-type', 'application/xml')
            self.end_headers()
            self.wfile.write(self.get_wsd_xml().encode())
            return
        if self.path == '/airmentorpro2.xml':
            self.send_response(200)
            self.send_header('Content-type', 'application/xml')
            self.end_headers()
    ...
    <device>
        <deviceType>urn:schemas-upnp-org:device:AirMentorPro2:1</deviceType>
        ...
        <serviceList>
            <service>
                <URLBase>https://www.air-mentor.com/web2017/product/air_mentor_8096ap</URLBase>
                <serviceType>urn:airmentor:service:AirMentorPro2:1</serviceType>
                <serviceId>urn:airmentor:serviceId:AirMentorPro2</serviceId>
                <controlURL>/AirMentorPro2</controlURL>
                <eventSubURL/>
                <SCPDURL>/AirMentorPro2_wsd.xml</SCPDURL>
            </service>
        </serviceList>
